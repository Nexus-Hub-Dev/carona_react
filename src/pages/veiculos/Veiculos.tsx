...
import {
  cadastrarVeiculo as cadastrarVeiculoApi,
  atualizarVeiculo as atualizarVeiculoApi,
  listarVeiculos,
  removerVeiculo as removerVeiculoApi,
...
function adaptarVeiculoSalvo(
  veiculo: Veiculo & { adaptadoPCD?: boolean },
): Veiculo {
  return {
    ...veiculo,
    acessivelPcd:
      veiculo.acessivelPcd ??
      veiculo.adaptadoPCD ??
      false,
  };
}
...
export function Veiculos() {
  ...
  const [veiculoEditando, setVeiculoEditando] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function carregarVeiculos() {
      ...
      try {
        ...
        const veiculosComSelecao = aplicarSelecaoLocal(
          veiculosApi.map(adaptarVeiculoSalvo),
          obterVeiculos().map(adaptarVeiculoSalvo),
        );
        ...
        /*
         * Se a API falhar, utiliza os veículos salvos localmente.
         */
        const veiculosLocais = obterVeiculos().map(adaptarVeiculoSalvo);

        if (montado) {
          setVeiculos(veiculosLocais);
          setErro(
            'Não foi possível carregar os veículos do servidor. Exibindo os dados locais.',
          );
        }
      } finally {
        if (montado) {
  ...
  async function cadastrarVeiculo(
    ...
    if (modeloNormalizado.length < 5) {
      ToastAlerta(
        'O modelo deve ter pelo menos 5 caracteres.',
        'erro',
      );
      return;
    }
     ...
     */
    const placaValida =
      /^[A-Z]{3}[0-9]{4}$/.test(placaNormalizada) ||
     ...
     */
    const placaJaCadastrada = veiculos.some(
      (veiculo) =>
        veiculo.placa
          .replace(/-/g, '')
          .toUpperCase() === placaNormalizada &&
        veiculo.id !== veiculoEditando,
    );
    ...
    if (corNormalizada.length < 3) {
      ToastAlerta(
        'A cor deve ter pelo menos 3 caracteres.',
        'erro',
      );
      return;
    }
    ...
    try {
      ...
      const destinoDepoisDoCadastro = (
        location.state as { from?: string } | null
      )?.from;

      if (destinoDepoisDoCadastro) {
        navigate(destinoDepoisDoCadastro, {
          replace: true,
        });
        return;
      }
      ...
      ToastAlerta(
        mensagem ||
          `Não foi possível salvar o veículo (${error?.response?.status ?? 'erro'}).`,
        'erro',
      );
    } finally {
      setSalvando(false);
  ...
  async function removerVeiculo(id: number) {
    ...
    try {
      ...
      const listaAtualizada = veiculos.filter(
    ...
    <section className="flex flex-1 justify-center bg-[#F6F3EB] px-4 py-10">
      <div className="w-full max-w-5xl">
          ...
          <div className="flex items-center justify-between gap-3 bg-black p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-300">
                Escolha qual carro está disponível para
                oferecer caronas.
              </p>
            </div>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="rounded-lg border border-gray-300 px-3 py-2 text-xs font-bold text-gray-800 hover:bg-gray-100"
                    >
                      Editar
                    </button>
        ...
        <button
          ...
          className="mt-6 flex w-full items-center justify-center rounded-2xl border border-black bg-black px-6 py-5 text-sm font-black text-white shadow-sm transition hover:bg-[#1d1d1d]"
        >
          + Adicionar veículo
        </button>
      </div>
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                aria-label="Fechar"
                className="grid h-9 w-9 place-items-center rounded-full text-xl text-gray-500 hover:bg-white hover:text-black"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-6 flex gap-2">
              ...
              <button
                type="button"
                onClick={limparFormulario}
                className="rounded-xl border border-gray-300 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-white"
              >
                Cancelar
              </button>
            </div>
